...
#ifdef SL_DIRECT_BOOT_LEVEL
// Sightline: boot straight into a stage instead of the title sequence.
//
// This is not a new feature - bossMainloop already handles g_StageNum being
// something other than the title, and does the whole setup itself: validates
// saves, selects folder 1, sets Agent difficulty, prepares the briefing, then
// calls lvlStageLoad. Setting the INITIALISER is what makes it reachable: the
// value is in place before the first instruction runs, so there is no window
// to hit and nothing for record and replay to disagree about.
//
// Rare's own route in is the -level_NN debug switch, but tokenReadIo reads
// that string over the dev-kit PI interface at 0xFFB000, which no ordinary
// emulator serves and which lies past the end of a retail image.
//
// Undefined by default, so the matching build is unaffected.
s32 g_StageNum = SL_DIRECT_BOOT_LEVEL;
#else
...
#else
    /* Sightline native: the N64 derivation spans _bssSegmentEnd to a TLB
     * block computed from the boot stack - memory-map arithmetic that is
     * meaningless (and was memory-corrupting) off-console. The platform
     * hands over a real heap instead. */
    {
        extern u8 *sl_native_heap(u32 *size);
        u32 sl_heap_size;
        start = sl_native_heap(&sl_heap_size);
        mempCheckMemflagTokens(start, sl_heap_size);
    }
#endif
...
#ifndef __sgi
    {
        // Sightline: replaying a recorded run requires the recorded RNG
        // state - the count-derived seed above is emulator boot timing.
        extern void sl_rng_seed_override(void);
        sl_rng_seed_override();
    }
#endif
