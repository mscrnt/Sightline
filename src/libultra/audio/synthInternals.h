...
#ifndef __sgi
    /* `force_aligned` states the intent: fccoef is DMA'd to the RSP and must be
     * 8-byte aligned, which an s64 member achieves on MIPS. It does NOT achieve
     * it here. On the i386 SysV ABI long long has 4-byte alignment INSIDE an
     * aggregate - gcc reports __alignof__(s64) as 8 but still packs it at 4 -
     * so this union landed at offset 4 instead of 8 and every emitted
     * coefficient-table address was exactly 4 bytes low. Measured against the
     * cartridge: all 350 LOADADPCM operands in the aligned tasks sat at -0x4.
     * Say the alignment explicitly rather than inferring it from a member type,
     * and keep the matching build's declaration byte-identical. */
