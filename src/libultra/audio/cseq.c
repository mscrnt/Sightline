...
#include "cseq.h"
/* relative: this file is built by src/libultrare/Makefile.libultrare, whose
 * include set is narrower than the main Makefile's and has no -I src/game */
#ifdef SIGHTLINE_AUDIO_EVENTS
#include "../../game/sl_audioev.h"
#endif
...
/* DURATION PARTITION. Counters only - the platform layer prints them.
 * Questions being separated, none of them assumed:
 *   - do bad durations cluster on a TRACK?
 *   - how many bytes does one varlen consume (a runaway read shows here)?
 *   - was the byte taken from the BACK-REFERENCE buffer or the main stream?
 * The input buffer is already proven byte-identical to the cartridge's, so the
 * fault is in consumption, and these are the three ways to consume wrongly. */
unsigned long sl_vl_bytes;             /* bytes used by the varlen in progress */
unsigned long sl_vl_frombu;            /* of those, taken from the backup buf  */
unsigned long sl_dur_hist[10];         /* bytes-per-duration histogram         */
unsigned long sl_dur_bad_bu;           /* big durations that used backup bytes */
unsigned long sl_dur_by_track[16];     /* note-ons per track                   */
unsigned long sl_dur_big_by_track[16]; /* of those, duration > 1536 ticks      */
unsigned long sl_dur_blockcode;        /* times the BLOCK_CODE path was taken  */
unsigned long sl_dur_esc;              /* of those, an escaped literal 0xFE    */
unsigned long sl_dur_val[64], sl_dur_trk[64], sl_dur_n;
#endif
...
void alCSeqNew(ALCSeq *seq, u8 *ptr)
{
    ...
    { extern void sl_seq_hdrstate(unsigned div, unsigned off0, unsigned off1);
      sl_seq_hdrstate(seq->base->division, seq->base->trackOffset[0],
                      seq->base->trackOffset[1]);
      extern void sl_seq_tracks(const unsigned *off, unsigned valid);
      sl_seq_tracks((const unsigned *) seq->base->trackOffset,
                    (unsigned) seq->validTracks); }
#endif
...
static u32 __alCSeqGetTrackEvent(ALCSeq *seq, u32 track, ALEvent *event) 
{
    ...
    else
    {
            ...
            ((event->msg.midi.status & 0xf0) != AL_MIDI_ChannelPressure))
        {
            ...
            {
#if defined(SL_ALHEAP_TRACE) && !defined(__sgi)
                { unsigned long b0 = sl_vl_bytes, u0 = sl_vl_frombu, n;
                  /* CURSOR BEFORE. __getTrackByte draws from the back-reference
                   * buffer while curBULen is nonzero and from curLoc otherwise,
                   * so the live cursor is whichever of the two the next byte
                   * will come from - reporting curLoc unconditionally would
                   * name the wrong pointer inside a back-reference. */
                  unsigned long cb = (unsigned long) (seq->curBULen[track]
                                       ? seq->curBUPtr[track] : seq->curLoc[track]);
                  unsigned long ca;
                  event->msg.midi.duration = __readVarLen(seq,track);
                  ca = (unsigned long) (seq->curBULen[track]
                                       ? seq->curBUPtr[track] : seq->curLoc[track]);
                  n = sl_vl_bytes - b0;
                  { extern void sl_noteon_record(unsigned trk, unsigned status,
                        unsigned key, unsigned vel, unsigned long curBefore,
                        unsigned long curAfter, unsigned bytes, unsigned long dur,
                        unsigned fromBackup);
                    sl_noteon_record((unsigned) track,
                        (unsigned) event->msg.midi.status,
                        (unsigned) event->msg.midi.byte1,
                        (unsigned) event->msg.midi.byte2,
                        cb, ca, (unsigned) n, (unsigned long) event->msg.midi.duration,
                        (unsigned) (sl_vl_frombu - u0)); }
                  sl_dur_hist[n < 10 ? n : 9]++;
                  if (sl_dur_n < 64) { sl_dur_val[sl_dur_n] = (unsigned long) event->msg.midi.duration;
                                       sl_dur_trk[sl_dur_n] = track; sl_dur_n++; }
                  if (track < 16) {
                      sl_dur_by_track[track]++;
                      if (event->msg.midi.duration > 1536) {
                          sl_dur_big_by_track[track]++;
                          if (sl_vl_frombu > u0) sl_dur_bad_bu++;
                      }
                  }
                }
#elif defined(SIGHTLINE_AUDIO_EVENTS)
                /* ROM EVENT PATH. Separate from the SL_ALHEAP_TRACE block above
                 * because the reference build defines only this one. The cursor
                 * is whichever pointer the next byte will come from, and both
                 * are logged so the host can compute the bytes consumed rather
                 * than the ROM formatting anything. */
                { u32 cb = (u32) (seq->curBULen[track]
                                  ? (u32) seq->curBUPtr[track]
                                  : (u32) seq->curLoc[track]);
                  u32 ca;
                  event->msg.midi.duration = __readVarLen(seq,track);
                  ca = (u32) (seq->curBULen[track]
                              ? (u32) seq->curBUPtr[track]
                              : (u32) seq->curLoc[track]);
                  SL_AEV(SL_AEV_NOTEON, track,
                         event->msg.midi.status & 0x0F,
                         event->msg.midi.byte1, event->msg.midi.byte2,
                         cb, ca, event->msg.midi.duration); }
#else
                event->msg.midi.duration = __readVarLen(seq,track);
#endif
#ifdef _DEBUG                
...
#if defined(SL_ALHEAP_TRACE) && !defined(__sgi)
    sl_vl_bytes++;
    if (seq->curBULen[track]) sl_vl_frombu++;
#endif
...
#if defined(SL_ALHEAP_TRACE) && !defined(__sgi)
            sl_dur_blockcode++;
            if (nextByte == AL_CMIDI_BLOCK_CODE) sl_dur_esc++;
#endif
