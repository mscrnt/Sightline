...
/* Voice-pool pressure. Stealing happens only when BOTH the lame list and the
 * free list are empty, so these three counts say whether the pool is under
 * pressure at all - which is what separates "native never needs to reclaim"
 * from "native reclaims differently". */
unsigned long sl_pv_lame, sl_pv_free, sl_pv_steal, sl_pv_stealfail;
#endif
s32 _allocatePVoice(ALSynth *drvr, PVoice **pvoice, s16 priority)
{
    ALLink      *dl;
...
#if defined(SL_ALHEAP_TRACE) && !defined(__sgi)
    if (!stolen && !sl_pv_lame && !sl_pv_free) sl_pv_stealfail++;
#endif
    return stolen;
}
