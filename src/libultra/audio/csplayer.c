...
/* NOTE-OFF PARTITION. Counters only - this file has the N64 stdio shadow, so
 * the platform layer prints them. Separates the three possibilities the
 * measurement has to decide between: note-offs NOT GENERATED (a note-on whose
 * duration is 0 schedules none at all - see `if (midi->duration)` below),
 * GENERATED AND DROPPED (scheduled but never processed, or processed with no
 * voice to stop), or GENERATED LATE (scheduled with too large a deltaTime,
 * which is uspt * duration). */
unsigned long sl_np_on, sl_np_off_sched, sl_np_zero_dur;
unsigned long sl_np_first_af, sl_np_first_set;
unsigned long sl_np_off_proc, sl_np_off_nullvoice;
unsigned long sl_np_uspt, sl_np_dur_min = 0xFFFFFFFFu, sl_np_dur_max;
unsigned long sl_np_dt_min = 0xFFFFFFFFu, sl_np_dt_max;
#endif
...
#if defined(SL_ALHEAP_TRACE) && !defined(__sgi)
                sl_np_on++;
                if (!midi->duration) sl_np_zero_dur++;
                if (!sl_np_first_set) { extern u32 g_AudioFrameCount;
                    sl_np_first_set = 1;
                    sl_np_first_af = (unsigned long) g_AudioFrameCount; }
#endif
                if(midi->duration)
                {
                    /*
...
#if defined(SL_ALHEAP_TRACE) && !defined(__sgi)
                    sl_np_off_sched++;
                    sl_np_uspt = (unsigned long) seqp->uspt;
                    if ((unsigned long) midi->duration < sl_np_dur_min)
                        sl_np_dur_min = (unsigned long) midi->duration;
                    if ((unsigned long) midi->duration > sl_np_dur_max)
                        sl_np_dur_max = (unsigned long) midi->duration;
                    if ((unsigned long) deltaTime < sl_np_dt_min)
                        sl_np_dt_min = (unsigned long) deltaTime;
                    if ((unsigned long) deltaTime > sl_np_dt_max)
                        sl_np_dt_max = (unsigned long) deltaTime;
#endif
