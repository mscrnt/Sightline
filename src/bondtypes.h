...
#include "snd.h"
/* Sightline: chrobjdata.h declares extern ARRAYS of ItemModelFileRecord and
 * ChrModelFileRecord; C requires complete element types for that, IDO never
 * enforced it. The include moved below those definitions (nothing in between
 * used its declarations - verified by symbol sweep). See the AIPARSE-guarded
 * include after ChrModelFileRecord. */
...
#else
/* Sightline native: IDO fills omitted arguments with nothing; a conforming
 * preprocessor refuses the short calls. Pad to three and swallow surplus. */
#define SL_New_Coord3d_IMPL(x, y, z, ...) \
    { IF_ELSE(IS_EMPTY(x))(0)(x), IF_ELSE(IS_EMPTY(y))(0)(y), IF_ELSE(IS_EMPTY(z))(0)(z) }
#define New_Coord3d(...) SL_New_Coord3d_IMPL(__VA_ARGS__, , ,)
#endif
...
#else
#define SL_New_Vector_IMPL(x, y, z, ...) \
    { IF_ELSE(IS_EMPTY(x))(0)(x), IF_ELSE(IS_EMPTY(y))(0)(y), IF_ELSE(IS_EMPTY(z))(0)(z) }
#define New_Vector(...) SL_New_Vector_IMPL(__VA_ARGS__, , ,)
#endif
...
#pragma region Stand Tiles
    ...
    */
    typedef struct StandTileHeaderMid
    {
        ...
        u16 b       : 4;
#else
        /* gcc allocates bitfields LSB-first; reversed so the fields match
         * the big-endian value semantics the raw `.half` readers use */
        u16 b       : 4;
        u16 g       : 4;
        u16 r       : 4;
        u16 special : 4;
    ...
    typedef struct StandTileHeaderTail
    {
        ...
        s16 headerE    : 4;
#else
        /* reversed for gcc; see StandTileHeaderMid */
        s16 headerE    : 4;
        s16 headerD    : 4;
        s16 headerC    : 4;
        s16 pointCount : 4;
...
#pragma region Prop, Chr and Definition Records
    ...
    */
    ...
    {
        inherits ObjectRecord;
        s32      lookpad; /* was `pad`; collides with ObjectRecord's own pad once
                             anonymous members are hoisted under -fms-extensions.
                             The comment always named it lookpad. Layout unchanged. */
        Mtxf     unk84;
        f32 unkC4;
        f32 unkC8;
        f32 unkCC;
