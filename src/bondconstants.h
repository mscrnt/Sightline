...
*/} NAME;
#else
/* Sightline native builds: IDO's preprocessor silently fills omitted macro
 * arguments with nothing, so 32-parameter BITFLAG is called with three args
 * all over this file. A conforming preprocessor refuses that, which is why
 * the reference build defined BITFLAG away entirely - taking PLAYERFLAG,
 * RUNTIMEBITFLAG and friends with it. Instead: pad the call out to 32
 * arguments and feed the SAME generator body, whose trailing ... swallows
 * the surplus. IDO never sees any of this.
 */
#define SL_BITFLAG_IMPL(NAME,a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q,r,s,t,u,v,w,x,y,z,aa,ab,ac,ad,ae,af,...)\
typedef enum NAME \
{ /*\
  */NAME ## _NONE,/*\
  */IF(NOT(IS_EMPTY(a)))(NAME ## _ ## a = 1 << 0) EVAL1(COMMA_IF_I(NOT(IS_EMPTY(b))))/*\
...
#define BITFLAG(NAME, ...) SL_BITFLAG_IMPL(NAME, __VA_ARGS__, \
    ,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,)
#endif
...
{
#ifndef __sgi
    /* SP_LEVEL_NONE exists to make this enum's UNDERLYING TYPE SIGNED, and it
     * is load-bearing rather than decorative.
     *
     * C leaves the underlying type implementation-defined. IDO picks int, so
     * on the matching build every LEVEL_SOLO_SEQUENCE comparison is signed.
     * GCC picks `unsigned int` whenever no enumerator is negative - which was
     * true here, DAM..MAX being 0..20 - and that silently changed the meaning
     * of two descending loops and four sentinel tests in file2.c/front.c.
     *
     * MEASURED in the shipped exe, file2.c:895
     * `for (stageid = SP_LEVEL_EGYPT; stageid >= SP_LEVEL_DAM; stageid--)`:
     *
     *   46cfb1:  subl $0x1,-0x4(%ebp)    ; stageid--
     *   46cfb5:  jmp  46cf82             ; back-edge, NO cmp, NO condition
     *
     * `unsigned >= 0` is a tautology, so GCC deleted the loop test outright.
     * The DIFFICULTY loop wrapping it kept its test (`cmpl $0x0` / `js`)
     * purely because DIFFICULTY_MULTI = -1 already forced that enum signed.
     * Same source shape, opposite codegen, and the only difference was the
     * presence of a negative enumerator.
     *
     * That infinite loop is what hung the front end: a folder with no
     * completed mission never hits the early return, so file select never
     * completes a frame. The last image actually presented is the preceding
     * GOLDENEYE_LOGO, which is why it was reported as a logo freeze.
     *
     * -1 is also the value file2.c already stores for "no stage completed"
     * (`*levelid = SP_LEVEL_DAM - 1`), so this names a sentinel that was
     * always there and was unrepresentable while the type was unsigned -
     * making `higheststage >= SP_LEVEL_DAM` wrongly TRUE for an empty folder.
     *
     * Guarded because the matching build must stay byte-identical; IDO needs
     * no help here and the original list follows verbatim under __sgi.
     * LEVEL_SOLO_SEQUENCE is never a struct member (tree-wide grep over src/
     * and include/: function parameters, returns and locals only), so nothing
     * changes size or layout - it is 4 bytes either way. */
    SP_LEVEL_NONE = -1,
#endif
    SP_LEVEL_DAM,
    SP_LEVEL_FACILITY,
...
#pragma region Object Instance Stuff
    ...
    {
#ifndef __sgi
        /* IDO gives enums signed int; gcc picks unsigned when every
         * enumerator is nonnegative, breaking `body < 0` sentinel checks.
         * A negative first enumerator forces signed (values unchanged). */
        SL_BODY_FORCE_SIGNED = -1,
#endif
...
#else
        /* -1 keeps enum HEADS signed under gcc; 0xFFFFFFFF forces the whole
         * enum unsigned and every `head >= 0` sentinel check goes wrong */
        HEAD_FIXED        = -1,
#endif
#ifdef __sgi
        HEAD_RANDOM       = 0xFFFFFF9F
#else
        HEAD_RANDOM       = -97 /* 0xFFFFFF9F; keeps the enum signed, see HEAD_FIXED */
#endif
    } HEADS;
...
#define MODELSKELETON(NAME, NUMJOINTS, SKELSIZE) ModelSkeleton SKELETON(NAME) = {NUMJOINTS, 0, JOINTLIST(NAME), SKELSIZE, 0}; /* Sightline: pasting against '(' is illegal outside IDO; the resulting token stream is identical without the ##s */
...
#define New_ModelFileHeader(NAME, SKELETONNAME, BOUNDINGRADIUS, NUMSWITCHES, HASNAMES, NUMMATRICES, NUMTEXTURES, NUMRECORDS, SWITCHES, ROOTNODE) \
    ...
    &SKELETON(SKELETONNAME), /* Sightline: '& ## x' is an illegal paste outside IDO; token stream unchanged */                                                                                                             \
...
#define CHRFILERECORD(NAME, SCALE, OFFSET, HASHEAD, ISMALE) \
    {&NAME##_header, STR(C##NAME##Z), SCALE, OFFSET, HASHEAD, ISMALE}, /* Sightline: dropped illegal '&##' paste */

#define GUNSTATS(NAME) &NAME##_stats /* Sightline: dropped illegal '& ##' paste */
#define GUNFILERECORD(NAME, NOMODEL, STATS, UPPERTEXTID, LOWERTEXTID, POSX, POSY, POSZ, XROT, YROT, WOCTEXT, EQUIPTEXT, EQUIPX, EQUIPY, EQUIPZ) \
    { &NAME##_header,STR(G## NAME ##Z), NOMODEL, STATS, UPPERTEXTID, LOWERTEXTID, POSX, POSY, POSZ, XROT, YROT, WOCTEXT, EQUIPTEXT, EQUIPX, EQUIPY, EQUIPZ}, /* Sightline: dropped illegal '& ##' paste */
#define SUIT_LFRECORD(NAME, NOMODEL, STATS, UPPERTEXTID, LOWERTEXTID, POSX, POSY, POSZ, XROT, YROT, WOCTEXT, EQUIPTEXT, EQUIPX, EQUIPY, EQUIPZ) \
    { &NAME##_header,STR(C## NAME ##Z), NOMODEL, STATS, UPPERTEXTID, LOWERTEXTID, POSX, POSY, POSZ, XROT, YROT, WOCTEXT, EQUIPTEXT, EQUIPX, EQUIPY, EQUIPZ}, /* Sightline: dropped illegal '& ##' paste */
/**
