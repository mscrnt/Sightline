...
#ifndef __sgi
    /* Native: the skeleton drives frames from the recv-pump and never runs the
     * scheduler, so osSpTaskStartGo is never reached and the DL would never be
     * seen. Hand it over here, where both ends of the list are in hand.
     * Declared, not included - src/game must not depend on src/gfx. */
    {
        extern void sl_gfx_frame_dl(const void *first, const void *end);
        sl_gfx_frame_dl(firstGdl, gdl);
    }
#endif
