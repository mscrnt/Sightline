...
void bondviewCalcIntroSwirlCamera(s32 index, f32 time, coord3d *pos, coord3d *lookat)
{
    ...
    f32 pointbuf[10];
#else
    /* the fourth spline control point is a coord3d at index 9 - the write
     * reaches index 11; the N64 frame absorbed the spill */
    f32 pointbuf[12];
#endif
...
#else
    /* stanTileDistanceRelated zeroes 16 words through the record pointer;
     * the N64 stack layout provided that room via the padding above, which
     * gcc does not guarantee - allocate it explicitly */
    struct {
        struct StandTileLocusCallbackRecord rec;
        s32 scratch[12];
    } sp3C_alloc;
#define sp3C sp3C_alloc.rec
#endif
 ...
 * @param collision_pt1: Out parameter. Will set {x,0,z}
#ifndef __sgi
#undef sp3C
#endif position of second point (from line edge) if Bond is in collision, otherwise ... Bond's look angle?
 *
...
#ifndef __sgi
/**
 * NATIVE ONLY - invisible to IDO, so the matching build is byte-identical.
 *
 * Two things the native keyboard/mouse override at the seam below needs, and
 * neither of them may exist for the matching build. Everything native in this
 * file is inside `#ifndef __sgi` for that reason: an earlier milestone lost
 * MATCH! to an inert `do{}while(0)` that IDO could still see, so the rule here
 * is not "harmless tokens are fine", it is ZERO tokens.
 *
 * sl_move_channels_get - the four movement channels the platform layer
 * publishes, in the game's own +/-70 unit. Defined in
 * src/native/sl_move_channels.c, which is compiled with the game include path
 * but is not game logic. Declared here rather than included, so that nothing
 * in src/game gains an include of src/platform and `make check-layering` is
 * untouched.
 *
 * g_sl_frozen_input - whether the CURRENT call to bondviewProcessInput came
 * from bondviewFrozenMoveBond (below), which passes an all-zero pad from the
 * cutscene/intro path. That path is chosen by g_CameraMode, NOT by the
 * controls-locked flag, so the gates the style branches use do not cover it:
 * an override that ignored this would walk Bond around during the level intro
 * camera. Set and cleared around the one call site.
 */
extern int sl_move_channels_get(s32 *walk, s32 *strafe, s32 *turn, s32 *pitch);

/* sl_mouse_look_get - the LINEAR look channel (src/native/sl_move_channels.c),
 * in DEGREES for this frame rather than the +/-70 stick unit the four above
 * use. Declared locally, inside this file's own #ifndef __sgi, exactly as the
 * line above is: IDO sees zero tokens and the layering rule is untouched.
 *
 * WHY IT BYPASSES THE CURVE BELOW. :6158-6178 divides analogTurn by 70, clamps
 * to +/-1 and SIGNED-SQUARES it. Squaring makes the total turn depend on how a
 * displacement was DISTRIBUTED ACROSS FRAMES, not on its sum, so equal-and-
 * opposite mouse motion delivered at different speeds does not return to the
 * starting angle - MEASURED at -52.5 degrees per cycle for 300 counts left
 * over 5 frames against 300 right over 20. That is the owner's reported
 * symptom. A stick reports a POSITION and the curve is right for it; a mouse
 * reports a DISPLACEMENT that has already happened. Returns 0 for the pad, for
 * every headless run and for every recorded replay, and the cartridge curve
 * then runs unchanged. */
extern int sl_mouse_look_get(f32 *yaw_deg, f32 *pitch_deg);

static s32 g_sl_frozen_input = 0;

/**
 * Whether the four keyboard/mouse channels were APPLIED on this tick - not
 * merely available. Set at the seam below, from the same condition, and read
 * once afterwards by the look-ahead auto-centre test.
 *
 * It exists because the seam is at :5406 and the auto-centre decision is at
 * :5911, five hundred lines later, and the two have to agree about whether the
 * player is steering. Clearing the auto-centre state AT the seam would simply
...
void bondviewProcessInput(s8 stick_x, s8 stick_y, u16 buttons, u16 oldbuttons)
{
...
#ifndef __sgi
    /* NATIVE KEYBOARD AND MOUSE - the seam, and the whole of it.
     *
     * WHERE. Immediately after the control-style branches above have finished
     * interpreting the pad, and before anything reads the four channels (the
     * earliest is :5537, analogStrafe). moveData is a LOCAL, so there is no
     * external struct to intercept and the override has to be here.
     *
     * WHY HERE AND NOT LATER. The invertPitch block a few lines down negates
     * analogPitch and swaps speedVertaUp/Down. Writing analogPitch BEFORE it
     * keeps the player's own Look Up/Down option working and leaves pitch
     * sense game-owned, rather than duplicating the option in the host.
     *
     * WHY HERE AND NOT EARLIER. Overriding after style interpretation is what
     * makes keyboard and mouse behave the same under Honey, Solitaire, Kissy
     * and Goodnight. The host does not know what a control style is and does
     * not need to; the styles keep their full meaning for the PAD, which does
     * not come through here at all.
     *
     * WHAT IT OWNS. Four numbers and their four enable gates - forward/back,
     * strafe, yaw, pitch. Nothing else. Acceleration curves, collision,
     * camera, look-ahead, auto-centring, pitch limits, aim mode, auto-aim,
     * firing, weapons, crouch, lean, the tank and the menus all stay exactly
     * where they were, and fire/aim/use/weapon-cycle still arrive as N64
     * BUTTONS interpreted by the selected style.
     *
     * THE GATES, and why each one is here rather than assumed:
     *
     *   g_sl_frozen_input      bondviewFrozenMoveBond calls this function with
     *                          an all-zero pad during intro cameras and other
     *                          frozen states (:7656). That path is selected by
     *                          g_CameraMode, so lvlGetControlsLockedFlag is
     *                          NOT guaranteed to be set there - without this
     *                          flag the override would walk Bond through his
     *                          own cutscenes.
     *   watch_animation_state  the watch is GoldenEye's pause and options
     *                          menu; the style branches above are inside the
     *                          same test (:4850).
     *   bonddead               likewise (:4851).
     *   lvlGetControlsLockedFlag / disablePlayerActionsWhenPausedOrInMpMenu
     *                          the two gates the 1.x branch uses at :5131-5133
     *                          and the 2.x branch at :4932.
     *   g_PlayerIsInTank       NOT a gate any more (2026-09-11, owner-reported:
     *                          in the Runway tank "looking around works but the
     *                          tank does not move with WASD"). The exclusion
     *                          was a deferral ("not this milestone's"), not a
     *                          correctness constraint, and the four writes
     *                          below are ALREADY tank-correct because they
     *                          speak through Rare's own channel flags, which
     *                          the tank consumption interprets tank-fashion:
     *                          canLookAhead + analogWalk is the tank's analog
     *                          DRIVE arm (:5739-5741, +/-TANK_MAX_SPEED), and
     *                          canTurnTank + analogStrafe its analog STEER arm
     *                          (:5716-5730) - exactly the flag state the 2.x
     *                          dual-analog styles produce in the tank on the
     *                          cartridge (:5012-5014), so nothing is invented.
     *                          Turn and pitch land in the same canNaturalTurn /
     *                          canNaturalPitch arms as on foot; the stick- and
     *                          C-derived rates those arms outrank are all zero
     *                          under a neutral keyboard/mouse virtual pad, the
     *                          same argument AIM MODE below already makes.
     *                          Before this, WASD fed nothing in the tank (the
     *                          keyboard synthesises no C buttons; only the
     *                          arrow-key d-pad, which the game ORs with C,
     *                          could drive it).
     *   insightaimmode         NOT a gate any more - see AIM MODE below. It
     *                          selects WHICH channels apply, not whether any
     *                          do.
     *
     * AIM MODE (changed 2026-09-03, owner-reported: right-click aiming
     * "behaves like a mouse being converted into digital/stick input").
     *
     * Aim mode was a gate here, and the platform layer routed the mouse onto
     * the N64 STICK for that window instead. That looked faithful and was not,
     * because of what the stick means while aiming: :6256 offsets the
     * CROSSHAIR by controlStickXRaw/YRaw, and :5309/:5318 only begin to turn
     * the view once |stick| passes 60, at a rate set by how far past 60 it
     * went. Those are readings of a stick POSITION. A mouse reports a RATE -
     * one poll's motion - so the reticle floated, the view did not move until
     * a single poll's delta cleared a hard threshold, and it stopped as a
     * pulse the instant the mouse did.
     *
     * So while aiming, the two LOOK channels apply and nothing else does:
     *
     *   analogTurn / analogPitch + canNaturalTurn / canNaturalPitch
     *                          the mouse turns and pitches the view exactly as
     *                          it does outside aim mode. canNaturalTurn takes
     *                          the FIRST arm at :6116, ahead of
     *                          aimTurnLeft/RightSpeed, and canNaturalPitch the
     *                          first arm at :6058, ahead of speedVertaUp/Down,
     *                          so the stick-extreme derivations are simply not
     *                          consulted - they are already 0, because the
     *                          keyboard/mouse stick is neutral.
     *   analogWalk / analogStrafe / canLookAhead / canTurnTank
     *                          NOT written while aiming. Aim mode stops the
     *                          player moving and disables look-ahead, and it
     *                          still does: these keep whatever the style
     *                          branches above left them.
     *
     * Everything else aim mode owns is untouched - entering and leaving it
     * (:5178 reads the aim BUTTON, which keyboard/mouse still sends as R and
     * this block never writes), auto-aim, zoom, the weapon state, the sights.
     *
     * LIVE KEYBOARD/MOUSE ONLY, and that is the whole safety argument: the pad
     * does not come through sl_move_channels_get at all, so a controller keeps
     * GoldenEye's original floating manual aim, unchanged. Recorded in
     * docs/divergences.md.
     *
     * sl_move_channels_get returns 0 whenever there is no live keyboard and
     * mouse - which is every headless run and every recorded replay - so the
     * block below cannot execute during trace verification at all. */
    {
        s32 sl_walk, sl_strafe, sl_turn, sl_pitch;

        g_sl_channels_live = 0;

        if (g_sl_frozen_input == 0
            && g_CurrentPlayer->watch_animation_state == WATCH_ANIMATION_0x0
            && g_CurrentPlayer->bonddead == FALSE
            && lvlGetControlsLockedFlag() == 0
            && disablePlayerActionsWhenPausedOrInMpMenu()
            && sl_move_channels_get(&sl_walk, &sl_strafe, &sl_turn, &sl_pitch))
        {
            moveData.analogTurn      = sl_turn;
            moveData.analogPitch     = sl_pitch;

            moveData.canNaturalTurn  = 1;
            moveData.canNaturalPitch = 1;

            if (g_CurrentPlayer->insightaimmode == 0)
            {
                moveData.analogWalk      = sl_walk;
                moveData.analogStrafe    = sl_strafe;
    ...
    if (g_CurrentPlayer->watch_animation_state == WATCH_ANIMATION_0x0)
    {
...
#ifndef __sgi
                /* NATIVE ONLY - zero tokens for IDO.
                 *
                 * The test above asks "is the player pitching?" by reading the
                 * DIGITAL C-button pitch rates. The keyboard/mouse seam writes
                 * analogPitch and canNaturalPitch and never touches speedVerta*,
                 * so for a mouse the answer came back NO however hard the player
                 * pitched. docentreupdown and automovecentre were therefore never
                 * cleared, :5940 re-armed docentreupdown on any walk past +/-60,
                 * and the block at :5965 then dragged vv_verta back toward
                 * targetPitch (about -4 degrees) - overriding the analog pitch
                 * branch entirely, because that branch is the ELSE of
                 * `if (docentreupdown)`. Owner: pitch "doesn't fully move up or
                 * down" and "fights to move away from centre of screen".
                 *
                 * Extending the CONDITION is the faithful repair. Writing the
                 * pitch channel into speedVertaUp/Down would be wrong twice over:
                 * those are the digital pitch RATE and feed the same integration
                 * the analog path already drives, so pitch would be applied
                 * twice. Nothing is invented here - the branch's own three
                 * assignments are reused, and clearing docentreupdown
                 * unconditionally is what handles the case where a previous
                 * frame had already latched it.
                 *
                 * INTENTIONAL DIVERGENCE, live keyboard/mouse only: look-ahead
                 * auto-centring is off for mouse users. It exists because a
                 * C-button pitch scheme cannot easily return to level, and it is
                 * actively wrong for a device that can. The pad keeps it - the
                 * pad does not come through the channels - and so do the matching
                 * build and every recorded replay, where g_sl_channels_live is
                 * always 0. Recorded in docs/divergences.md. */
                || g_sl_channels_live
#endif
                )
            {
        ...
        else
        {
            if (moveData.canNaturalPitch)
            {
                ...
                /* LINEAR MOUSE PITCH, mirroring the yaw branch: :6146
                 * integrates `vv_verta += speedverta * g_GlobalTimerDelta *
                 * 3.5f`, so the same division lands on exactly sl_pitch_deg.
                 *
                 * NOTHING IS INVERTED HERE. The channel is already + = look UP
                 * and final; the Look Up/Down controller option is deliberately
                 * not applied to the mouse (the standing "THE MOUSE OWNS ITS
                 * OWN PITCH" decision), which is why this does not mirror the
                 * invertPitch negation at :5618 that analogPitch goes through.
                 *
                 * The +/-90 clamp is NOT bypassed: it is applied to vv_verta
                 * itself at :4696-4700, downstream of every producer, so
                 * linearising the rate cannot look past vertical or wrap. */
                if (sl_mouse_look_get(NULL, &sl_pitch_deg))
                {
                    g_CurrentPlayer->speedverta = (g_GlobalTimerDelta > 0.0f)
                        ? (sl_pitch_deg / (g_GlobalTimerDelta * 3.5f))
                        : 0.0f;
                }
                else
                {
    ...
    if (moveData.canNaturalTurn)
    {
        ...
        /* LINEAR MOUSE YAW. speedtheta is a RATE that :6951 integrates as
         * `vv_theta += speedtheta * g_GlobalTimerDelta * 3.5f`, so dividing the
         * degrees wanted this frame by that same factor makes the integration
         * land on exactly sl_look_deg - no curve, no ceiling, no carry, and
         * frame-rate independent because the delta cancels. speedtheta keeps
         * its normal meaning downstream, so gun sway (:6202, :6275) still
         * tracks the turn instead of going flat. */
        if (sl_mouse_look_get(&sl_look_deg, NULL))
        {
            g_CurrentPlayer->speedtheta = (g_GlobalTimerDelta > 0.0f)
                ? (sl_look_deg / (g_GlobalTimerDelta * 3.5f))
                : 0.0f;
        }
        else
        {
...
#else
        /* stanTileDistanceRelated zeroes SIXTEEN words through this pointer
         * (stan.c:2421, the `for(i=0;;)` HACK loop), and
         * struct move_bond_temp_struct is TWO. The N64 stack layout supplied
         * the other fourteen; gcc does not guarantee it, so the write ran off
         * the end of curLocus and into MoveBond's neighbouring locals.
         *
         * MEASURED: sp200 held a valid tile (20125c04) immediately BEFORE the
         * first stanTileDistanceRelated call and arrived NULL inside it, so
         * sub_GAME_7F0B1DDC stored a null tileStack[0] and stanIsSpecialBit1Set
         * faulted reading arg0->mid.half at offset 4. Facility, frame 567,
         * deterministic. B-068.
         *
         * Same defect and same fix as sp3C in
         * bondviewCalcUpdatePlayerCollision above - see the note there. */
        struct {
            struct move_bond_temp_struct rec;
            s32 scratch[14];
        } curLocus_alloc;
#define curLocus curLocus_alloc.rec
#endif
...
void bondviewFrozenMoveBond(s8 stick_x, s8 stick_y, u16 buttons, u16 oldbuttons)
{
    ...
    bondviewPlayerTickExplode();
#ifndef __sgi
    /* Tell the seam in bondviewProcessInput that THIS call is the frozen one.
     * The all-zero pad is the tell for a real controller, but the native
     * override does not come through the pad, so it needs to be told. */
    g_sl_frozen_input = 1;
#endif
...
s32 playerTick(PropRecord *prop)
{
    ...
    f32 mtx[15];
#else
    /* matrix_4x4_multiply_homogeneous writes a full 4x4 - 16 floats; the
     * N64 frame absorbed the 16th, the canary does not */
    f32 mtx[16];
#endif
    s32 tailret;
    s32 anim;
