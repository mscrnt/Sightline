...
void setupCctv(s32 arg0, CCTVRecord *arg1, s32 cmdindex)
{
    ...
    if (arg1->lookpad >= 0)
    {
        ...
        if (isNotBoundPad(arg1->lookpad))
        {
            sp50 = &g_CurrentSetup.pads[arg1->lookpad];
        }
        else
        {
            sp50 = (struct PadRecord *)&g_CurrentSetup.boundpads[getBoundPadNum(arg1->lookpad)];
        }
...
#ifndef __sgi
/* T4: stage setup files are big-endian.  Swapped once at load, per section:
 * the 10 header offsets, waypoint/waygroup/patrol records and their -1
 * terminated ID arrays, the ailist directory, and the all-word pad and
 * boundpad sections (bounded by the nearest following section).  The name
 * sections swap only their offset arrays (strings are bytes).  propDefs is
 * type-dependent and gets its own pass when its readers arrive. */
static u32 sl_susw(u32 v)
{
    return (v >> 24) | ((v >> 8) & 0xFF00) | ((v << 8) & 0xFF0000) | (v << 24);
}

static void sl_su_words(u8 *base, u32 off, u32 nwords)
{
    u32 *w = (u32 *) (base + off);
    u32 i;
...
static void sl_su_idarray(u8 *base, u32 off)
{
    u32 *w = (u32 *) (base + off);
...
/* The nearest section start after `from` - the ten header sections, plus
 * `extra` when it is a data block the header does not name (0 = none). */
static u32 sl_su_next(u32 *offs, u32 from, u32 extra)
{
    u32 best = 0;
    s32 i;

    for (i = 0; i < 10; i++)
        if (offs[i] > from && (best == 0 || offs[i] < best))
            best = offs[i];
    if (extra > from && (best == 0 || extra < best))
        best = extra;
    return best;
}

/* The credits table an INTROTYPE_CREDITS record points at: 0xC-byte records
 * of six 16-bit fields (bondtypes.h CreditsEntry; the cartridge's intro
 * handler reads them LHU at +0 and +2 on a 0xC stride, "Intro Block
 * Commands.txt" type 8), ended by a record whose two text ids are both 0.
 * It is not one of the ten header sections, so it has to be swapped as
 * halfwords here and kept OUT of the all-word span swaps - on Cuba it sits
 * between boundpads and propDefs, and the boundpads span swap used to run
 * over it as 32-bit words, exchanging every pair of fields (text ids,
 * position/alignment): "The Cast" drawn from its centre and every cast row's
 * name and role both starting at x 0/1 instead of 250 / right-aligned to 190. */
static void sl_su_credits(u8 *base, u32 off)
{
    u16 *h = (u16 *) (base + off);
...
static void sl_setup_file_swap(void *file)
{
    u8 *base = file;
    u32 *w = file;
    u32 offs[10];
    ...
    /* pathwaypoints: 4-word records until padID == -1 */
    if ((off = offs[0]) != 0) {
        u32 *rec = (u32 *) (base + off);
        for (; rec[0] != 0xFFFFFFFF; rec += 4) {
            rec[0] = sl_susw(rec[0]);
            rec[1] = sl_susw(rec[1]);
            rec[2] = sl_susw(rec[2]);
            rec[3] = sl_susw(rec[3]);
            if (rec[1])
                sl_su_idarray(base, rec[1]);
        }
    }

    /* waypointgroups: 3-word records until neighbours == 0 */
    if ((off = offs[1]) != 0) {
        u32 *rec = (u32 *) (base + off);
        for (; *(volatile u32 *) rec != 0; rec += 3) {
            rec[0] = sl_susw(rec[0]);
            rec[1] = sl_susw(rec[1]);
            rec[2] = sl_susw(rec[2]);
            if (rec[0])
                sl_su_idarray(base, rec[0]);
            if (rec[1])
                sl_su_idarray(base, rec[1]);
        }
    }

    /* propDefs: typed records, per-type swappers in sl_swap_setup.c */
    if ((off = offs[3]) != 0) {
        extern void sl_propdefs_swap(void *propDefs);
        sl_propdefs_swap(base + off);
    }

    /* intro: typed records, walked exactly as the reader walks them
     * (a blanket span swap would clobber the waygroup ID arrays that
     * share the gap before the next section) */
    if ((off = offs[2]) != 0) {
        u32 *rec = (u32 *) (base + off);
        s32 done = 0;
        ...
        while (!done) {
            ...
            rec[0] = t;
            switch (t) {
                case INTROTYPE_SPAWN:   nwords = 3;  break;
                case INTROTYPE_ITEM:    nwords = 4;  break;
                case INTROTYPE_AMMO:    nwords = 4;  break;
                case INTROTYPE_SWIRL:   nwords = 8;  break;
                case INTROTYPE_ANIM:    nwords = 2;  break;
                case INTROTYPE_CUFF:    nwords = 2;  break;
                case INTROTYPE_CAMERA:
                    /* words through unk18, then lang1c as two u16s,
                     * then lang20 and the runtime prev word */
                    for (i = 1; i < 7; i++)
                        rec[i] = sl_susw(rec[i]);
                    *(u16 *) (rec + 7)       = (u16) ((*(u16 *) (rec + 7) >> 8) | (*(u16 *) (rec + 7) << 8));
                    *((u16 *) (rec + 7) + 1) = (u16) ((*((u16 *) (rec + 7) + 1) >> 8) | (*((u16 *) (rec + 7) + 1) << 8));
                    rec[8] = sl_susw(rec[8]);
                    rec[9] = sl_susw(rec[9]);
                    rec += 10;
                    continue;
                case INTROTYPE_WATCH:   nwords = 3;  break;
                case INTROTYPE_CREDITS: nwords = 2;  break;
                case INTROTYPE_END:
                default:                nwords = 1;  done = 1; break;
            }
            for (i = 1; i < (s32) nwords; i++)
                rec[i] = sl_susw(rec[i]);
            if (t == INTROTYPE_CREDITS && rec[1] != 0 && credits_off == 0)
                credits_off = rec[1];
            rec += nwords;
        }
    }

    /* credits table (see sl_su_credits): halfwords, before the span swaps
     * below so its bound is known to them */
    if (credits_off != 0)
        sl_su_credits(base, credits_off);

    /* patrolpaths: {offset, u8, u8, u16} until offset == 0 */
    if ((off = offs[4]) != 0) {
        u8 *rec = base + off;
        for (; *(u32 *) rec != 0; rec += 8) {
            *(u32 *) rec = sl_susw(*(u32 *) rec);
            *(u16 *) (rec + 6) = (u16) ((*(u16 *) (rec + 6) >> 8) | (*(u16 *) (rec + 6) << 8));
            sl_su_idarray(base, *(u32 *) rec);
        }
    }

    /* ailists: 2-word records until ailist == 0 (scripts are byte streams) */
    if ((off = offs[5]) != 0) {
        u32 *rec = (u32 *) (base + off);
        for (; rec[0] != 0; rec += 2) {
            rec[0] = sl_susw(rec[0]);
    ...
    /* pads (0x2C) and boundpads (0x44): all-word records to the next section
     * (or to the credits table, when that comes first) */
    if ((off = offs[6]) != 0 && (next = sl_su_next(offs, off, credits_off)) != 0)
        sl_su_words(base, off, (next - off) / 4);
    if ((off = offs[7]) != 0 && (next = sl_su_next(offs, off, credits_off)) != 0)
        sl_su_words(base, off, (next - off) / 4);

    /* pad name offset arrays: zero-terminated (see the promote walk) */
    if ((off = offs[8]) != 0) {
        u32 *rec = (u32 *) (base + off);
        for (; *rec != 0; rec++)
            *rec = sl_susw(*rec);
...
#ifndef __sgi
        sl_setup_file_swap(g_ptrStageSetupFile);
#endif
