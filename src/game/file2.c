...
void fileValidateSaves(void)
{
    ...
    if (fileGamePakProbe())
    {
...
#ifndef __sgi
        /* EEPROM images are cartridge byte dumps.  The stored checksums are
         * compared as VALUES against fileGenerateCRC's output, so they must be
         * promoted from big-endian first; the CRC's own input range is read
         * byte-wise and stays in wire order. */
        joyChecksum.chksum1 = (int) sl_be32r(&joyChecksum.chksum1);
        joyChecksum.chksum2 = (int) sl_be32r(&joyChecksum.chksum2);
#endif
...
#ifndef __sgi
        /* Same promotion for the five folder saves.  Left unswapped, every
         * checksum mismatched and fileResetSave() replaced the player's saved
         * settings with DEFAULT_OPTIONS - which silently dropped the recorded
         * session's inverted-look setting and made Bond walk differently the
         * first time the stick moved. */
        {
            s32 sv;
            for (sv = 0; sv < 5; sv++) {
                saves[sv].chksum1 = (s32) sl_be32r(&saves[sv].chksum1);
                saves[sv].chksum2 = (s32) sl_be32r(&saves[sv].chksum2);
            }
        }
#endif
        ...
        for (i = SAVESLOT1; i != SAVESLOTRAMROM; i++) //only != matches
        {
            ...
            {
                /* Survived validation, so the payload is still in cartridge
                 * order: promote the one multi-byte field read as a value.
                 * A reset save already holds native-order defaults. */
                saves[i].options = sl_be16r(&saves[i].options);
            }
#endif
        }
