...
#ifndef __sgi
/* T4: language files are a u32 offset table (BE) at the head — table extent
 * is the first string's offset — followed by raw string bytes.  Swap the
 * table words exactly once, at load. */
static void sl_lang_swap(s32 bank)
{
    u32 *w = (u32 *) bank;
    u32 limit = 0;
    u32 i, v;
    ...
    for (i = 0; i < 0x400; i++) {
        v = (w[i] >> 24) | ((w[i] >> 8) & 0xFF00) | ((w[i] << 8) & 0xFF0000) | (w[i] << 24);
        if (v != 0) {
            limit = v;
            break;
        }
...
static void sl_lang_swap_all(void)
{
    s32 i;

    for (i = 0; i < 45; i++)
        sl_lang_swap(g_LangBanks[i]);
}
#endif
...
#ifndef __sgi
    /* an unloaded bank on N64 reads through the cartridge TLB window (the
     * ROM header, effectively); natively there is nothing there */
