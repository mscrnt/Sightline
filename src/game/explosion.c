...
#ifndef __sgi
/* ==================== B-051/B-032 DECAL PROBE - INVESTIGATION ONLY =======
 *
 * NATIVE ONLY, DEFAULT OFF, NEVER COMMITTED. Armed by SL_DECAL_DBG.
 *
 * The question: a bullet decal can fail in three distinguishable places and
 * the fix is different for each.
 *
 *   NOT SPAWNED     explosionCreateBulletImpact never runs for this shot.
 *   NOT SUBMITTED   the entry exists but explosionRenderBulletImpactOnProp
 *                   skips it, and there are exactly three gates that can do
 *                   that: room >= 0, (prop != NULL || getROOMID_isRendered),
 *                   and the opa/xlu pass split on g_ImpactTypes[].unk1/unk2.
 *   NOT VISIBLE     the quad is in the display list and the renderer does
 *                   not show it.
 *
 * This probe separates the first two and counts what reaches the third. Every
 * gate is counted SEPARATELY - a single "skipped" number cannot tell a room
 * gate from a pass gate, and the whole point is which one fires.
 *
 * READ BEFORE BELIEVING A ZERO: the create counter is the positive control.
 * If creates == 0 the run never shot anything and nothing downstream means
 * anything. If creates > 0 and submitted == 0, the gate columns name which.
 *
 * It is read-only: no field is written and no branch outside the block moves.
 */
...
static int sl_decal_hide(void)
{
    static int on = -1;
    if (on < 0) on = (int) sl_env_f32("SL_DECAL_HIDE", 0.0f);
    return on;
}

/* per-frame counters for the render walk */
static int sld_calls, sld_bgcalls, sld_seen, sld_match, sld_roomok,
           sld_rendok, sld_roomrej, sld_passok, sld_quads;
static int sld_creates;                 /* cumulative, the positive control */
static int sld_lastframe = -1;

static void sl_decal_frame_report(void)
{
    if (!sl_decal_on()) return;
    if (sl_dbg_frame == sld_lastframe) return;
    if (sld_lastframe >= 0 && (sld_match | sld_quads | sld_roomrej)) {
        fprintf(stderr,
            "sl_decal: f%d read=%u calls=%d(bg=%d) entries-matched=%d"
            " room>=0:%d rendered-ok:%d ROOM-REJECTED:%d pass-ok:%d"
            " QUADS=%d  creates-so-far=%d\n",
            (int) sld_lastframe, sl_record_index(), sld_calls, sld_bgcalls,
            sld_match, sld_roomok, sld_rendok, sld_roomrej, sld_passok,
            sld_quads, sld_creates);
    }
    sld_lastframe = sl_dbg_frame;
    sld_calls = sld_bgcalls = sld_seen = sld_match = sld_roomok = 0;
    sld_rendok = sld_roomrej = sld_passok = sld_quads = 0;
}
#endif
...
void explosionCreateBulletImpact(struct coord3d *pos, struct coord3d *arg1, s16 impact_type, s16 room, PropRecord *prop, s8 model_render_pos_index, s8 room_clear_flag)
{
    ...
    if (sl_decal_on()) {
        sld_creates++;
        fprintf(stderr, "sl_decal: CREATE f%d read=%u type=%d room=%d"
                        " prop=%p mrpi=%d clearflag=%d pos=(%.1f,%.1f,%.1f)"
                        " slot=%d\n",
                (int) sl_dbg_frame, sl_record_index(), (int) impact_type,
                (int) room, (void *) prop, (int) model_render_pos_index,
                (int) room_clear_flag, pos->f[0], pos->f[1], pos->f[2],
                (int) g_NumImpactEntries);
        if (prop != NULL && prop->obj != NULL)
            fprintf(stderr, "sl_decal:   attached to PROPDEF=%d flags=%08x"
                            " state=%02x unk1=%d unk2=%d\n",
                    (int) prop->obj->type, (unsigned) prop->flags,
                    (unsigned) (prop->obj->state & 0xff),
                    (int) g_ImpactTypes[impact_type].unk1,
                    (int) g_ImpactTypes[impact_type].unk2);
    }
#endif
...
#ifndef __sgi
    sl_decal_frame_report();
    if (sl_decal_on()) { sld_calls++; if (arg1 == NULL) sld_bgcalls++; }
#endif
    for (i = 0; i < BULLET_IMPACT_BUFFER_LEN; i++)
    {
        ...
        {
#ifndef __sgi
            if (sl_decal_on() && g_BulletImpactBuffer[i].room >= 0
                              && arg1 == NULL) {
                sld_match++;
                sld_roomok++;
                if (getROOMID_isRendered(g_BulletImpactBuffer[i].room))
                    sld_rendok++;
                else {
                    sld_roomrej++;
                    if (sl_decal_on() >= 2)
                        fprintf(stderr, "sl_decal: f%d ROOM-REJECT slot=%d"
                                        " room=%d type=%d\n",
                                (int) sl_dbg_frame, (int) i,
                                (int) g_BulletImpactBuffer[i].room,
                                (int) g_BulletImpactBuffer[i].impact_type);
                }
            } else if (sl_decal_on() && g_BulletImpactBuffer[i].room >= 0) {
                sld_match++; sld_roomok++; sld_rendok++;
            }
#endif
            if (g_BulletImpactBuffer[i].room >= 0)
            {
                if (arg1 || getROOMID_isRendered(g_BulletImpactBuffer[i].room))
                {
                    ...
                    {
#ifndef __sgi
                        if (sl_decal_on()) { sld_passok++; sld_quads++; }
                        /* A/B control: emit everything EXCEPT the two
                         * triangles.  Two runs that differ only in this
                         * answer "does the decal put pixels on screen".
                         * The matrix, texture and vertex commands still go
                         * out, so only the rasterisation is removed. */
                        if (sl_decal_hide()) { sp50 = 1; continue; }
#endif
                        sp50 = 1;
