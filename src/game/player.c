...
#ifdef SL_BOND_HEALTH_MULT
    // Sightline: observation recordings. A deep health pool lets a whole level
    // be walked on any difficulty to watch spawn points, guard accuracy, alarm
    // propagation and squad reaction, without dying limiting the coverage.
    //
    // Deliberately NOT the invincibility cheat. That flag is tested after
    // damage_dealt is computed, so the game still registers hits - but it
    // skips deducting health, and health is what the trace records. Accuracy
    // would become invisible in exactly the recordings made to measure it.
    // Scaling health instead keeps every hit observable as a decrement.
    //
    // actual_health is Rare's own multiplier; their cheats set it to 2 and 10.
    //
    // Undefined by default; the matching build is unaffected. Traces recorded
    // this way are AI OBSERVATION traces, not parity traces.
    g_playerPointers[player_num]->actual_health = (f32)SL_BOND_HEALTH_MULT;
#else
#ifdef __sgi
    g_playerPointers[player_num]->actual_health = 1.0f;
#else
    /* native runtime equivalent of the SL_BOND_HEALTH_MULT build define
     * (TOUGH= in the recording sidecars) */
    {
        extern float sl_env_f32(const char *name, float dflt);
        g_playerPointers[player_num]->actual_health = sl_env_f32("SL_TOUGH", 1.0f);
    }
#endif
#endif
