...
#else
    /* `stage` is a LEVELID - see the parameter type, and every caller.
     * pull_and_display_text_for_folder_a0 is keyed by SP_LEVEL. B-096.
     * sl_briefing_page_for_stage_id (front.c) scans the same table by the
     * column that matches this domain. */
    briefingpage = sl_briefing_page_for_stage_id(stage);
#endif
}
