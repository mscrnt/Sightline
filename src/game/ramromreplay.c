...
#ifndef __sgi
/* IDO pads ramromfilestructure to 8-byte alignment (u64 members): its
 * sizeof is 232, the stride the N64-recorded demo streams were written
 * with.  gcc/i386 aligns u64 to 4 and gets 228 - a silent 4-byte stream
 * desync.  Field offsets are identical; only the stride differs. */
#define SL_SIZEOF_RAMROMFILE 232
#endif
...
void load_ramrom_from_devtool(void)
{
    ...
    if (indycommHostCheckFileExists(&strDemoFileName, &size) != 0)
    {
        ...
        ptr_active_demofile = romCopyAligned(&ramrom_data_target, (u8 *)INDY_RAMROM_DEMO_ADDRESS, 
#ifdef __sgi
            sizeof(struct ramromfilestructure)
#else
            SL_SIZEOF_RAMROMFILE
#endif
            );
    }
}
...
#else
        address_demo_loaded += SL_SIZEOF_RAMROMFILE;
#endif
...
#ifndef __sgi
/* T4: demo files are big-endian on cartridge; swap the header once at load.
 * The savefile blob is byte-oriented and left as-is; the command stream is
 * handled by the replay reader. */
static void sl_ramrom_header_swap(ramromfilestructure *f)
{
    s32 i;
    u8 *b;
    u8 t;

    /* the two u64 seeds: reverse all 8 bytes in place */
    for (b = (u8 *) &f->randomseed, i = 0; i < 4; i++) {
        t = b[i]; b[i] = b[7 - i]; b[7 - i] = t;
    }
    for (b = (u8 *) &f->randomizer, i = 0; i < 4; i++) {
        t = b[i]; b[i] = b[7 - i]; b[7 - i] = t;
    }

#define SL_RR32(fld) (*(u32 *) &(fld) = \
        ((*(u32 *) &(fld)) >> 24) | (((*(u32 *) &(fld)) >> 8) & 0xFF00) | \
        (((*(u32 *) &(fld)) << 8) & 0xFF0000) | ((*(u32 *) &(fld)) << 24))
    SL_RR32(f->stagenum);
    SL_RR32(f->difficulty);
    SL_RR32(f->size_cmds);
    SL_RR32(f->totaltime_ms);
    SL_RR32(f->filesize);
    SL_RR32(f->mode);
    SL_RR32(f->slotnum);
    SL_RR32(f->numplayers);
    SL_RR32(f->scenario);
    SL_RR32(f->mpstage_sel);
    SL_RR32(f->gamelength);
    SL_RR32(f->mp_weapon_set);
    for (i = 0; i < 4; i++) {
        SL_RR32(f->mp_char[i]);
        SL_RR32(f->mp_handi[i]);
        SL_RR32(f->mp_contstyle[i]);
        SL_RR32(f->mp_flags[i]);
    }
    SL_RR32(f->aim_option);
#undef SL_RR32
}
#endif
...
void replay_recorded_ramrom_at_address(ramromfilestructure *demofile)
{
    ...
    ptr_active_demofile = romCopyAligned(&ramrom_data_target, address_demo_loaded, 
#ifdef __sgi
            sizeof(struct ramromfilestructure)
#else
            SL_SIZEOF_RAMROMFILE
...
#ifndef __sgi
    sl_ramrom_header_swap(ptr_active_demofile);
#endif
#ifdef __sgi
