...
/* THE DEMO ACCESS POLICY, in the three places this file needs it.
 *
 * SL_DEMO_BUILD is a BUILD-TIME identity, set only by
 * `tools/windows/build.ps1 -Demo`. There is deliberately no environment
 * variable behind it: the shipped demo must behave identically however it is
 * launched, and a runtime knob the player could unset is not a policy. A
 * tree-wide `grep -rn SL_DEMO src/` finds only these compile-time tests and no
 * getenv.
 *
 * The demo is the FULL SINGLE-PLAYER CAMPAIGN with the cheat menu open and
 * Multiplayer closed:
 *
 *   missions      every MISSION_PART row of mission_folder_setup_entries is
 *                 available, Aztec and Egypt included, without the
 *                 progression the cartridge would demand
 *                 (get_highest_unlocked_difficulty_for_level)
 *   cheats        every cheat the cartridge can EARN is unlocked in the cheat
 *                 menu; NONE is active until the player switches it on there
 *                 (frontCheckIfCheatIsUnlocked)
 *   multiplayer   visible, dimmed, never selectable
 *                 (SL_DEMO_MULTIPLAYER_SELECTABLE below)
 *
 * Everything else is the game: the debrief walks the campaign on exactly as
 * Rare wrote it, completions and best times are recorded into the demo's own
 * save (src/platform/sl_ultra_shim.c, sl_demo_save_path), and nothing here
 * writes progression to make a row appear. Everything below is presentation
 * and ACCESS only. No game system is removed and a build without this define
 * is unchanged. */
...
#if defined(SL_DEMO_BUILD) && !defined(__sgi)
/* DEMO ACCESS POLICY - EVERY CHEAT THE CARTRIDGE CAN EARN IS UNLOCKED.
 *
 * The switch below is the game's own answer to "is this cheat in the cheat
 * menu": its first group returns 0 for the button-code and debug cheats the
 * menu never lists, and every other arm asks the SAVE whether the cheat has
 * been earned - twenty by fileGetIsCheatUnlocked against the mission whose
 * target time awards it, three by the Cradle / Aztec / Egypt completion
 * tests. Twenty-three menu cheats, and the switch is the only place that set
 * is written down.
 *
 * So the demo keeps the switch VERBATIM and changes only what the save
 * answers, for the length of this one function: the four save queries are
 * shadowed by TRUE here and unshadowed immediately after the closing brace.
 * The classification of which cheats are menu cheats is untouched and no
 * second list exists to drift from it. Tree-wide grep for the four names:
 * this function is their ONLY use in front.c; file.c:48 (recording a fresh
 * unlock at the debrief) and file2.c (the multiplayer-character and 007
 * unlocks) are other translation units and still read the save.
 *
 * UNLOCKED IS NOT ACTIVE. This function feeds cheat_available[] and
 * is_cheat_menu_available (interface_menu06_modesel), which decide what the
 * cheat menu LISTS. Activation is g_CheatActivated[], written only by the
 * cheat menu's own toggle (interface_menu15_cheat) and cleared on every entry
 * to File Select (init_menu05_fileselect); nothing here touches it, so a demo
 * boot starts with every cheat OFF and the player switches them on the way
 * the cartridge always has. Tree-wide `grep -rn g_CheatActivated src/`: the
 * writers are init_menu05_fileselect (the clear), interface_menu15_cheat (the
 * toggle) and src/native/sl_cheat.c (the developer SL_CHEATS knob), none of
 * them demo policy. The matching build is unchanged: with __sgi defined these
 * directives vanish and the function preprocesses to the same text as before. */
#define fileGetIsCheatUnlocked(save, mission)           (TRUE)
#define fileIsCradleCompletedForFolder(folder)          (TRUE)
#define fileIsAztecCompletedOnSecretOr00ForFolder(f)    (TRUE)
#define fileIsEgyptCompletedOn00ForFolder(folder)       (TRUE)
#endif
...
#if defined(SL_DEMO_BUILD) && !defined(__sgi)
/* End of the demo's shadowing; every later use of these names is the real
 * function again. See the block above frontCheckIfCheatIsUnlocked. */
#undef fileGetIsCheatUnlocked
#undef fileIsCradleCompletedForFolder
#undef fileIsAztecCompletedOnSecretOr00ForFolder
#undef fileIsEgyptCompletedOn00ForFolder
#endif
...
#ifndef __sgi
    /* NATIVE ONLY - the PC mouse, moving the cursor this function integrates.
     *
     * GoldenEye's front end is a pointer menu already: this ONE cursor is what
     * every front-end screen hit-tests, and the highlight each screen draws
     * comes out of that hit test. So a native absolute pointer needs no hover
     * state and no second highlight - it needs to put this cursor where the
     * pointer is, and everything downstream stays Rare's.
     *
     * AT THE TOP, deliberately, for two reasons. The clamps at the end of this
     * function then bound the pointer exactly as they bound the stick, so the
     * mapping carries no margin constants of its own; and the stick integrates
     * ON TOP, so a player using both at once gets the stick's delta applied to
     * the pointer's position rather than the two fighting.
     *
     * It is a no-op unless live keyboard/mouse input is running against a real
     * window AND the pointer has actually moved since the last frame - so the
     * pad and the keyboard keep the cursor whenever the mouse is still, and
     * trace replay, headless runs and demo playback never reach it at all. See
     * src/native/sl_menu_pointer.c. */
    {
        extern void sl_menu_pointer_apply(void);
        sl_menu_pointer_apply();
    }
#endif
...
//********************************************************************************************************
#ifndef __sgi
/* VI periods the cartridge spends inside init_menu00_legalscreen before the
 * first interface call can run. Native pays none of it; see the note at the
 * bottom of that function for the measurement this comes from. */
#define MENU_LEGALSCREEN_ENTRY_VI_COST 173
#endif
...
#ifndef __sgi
    /* CHARGE THE CARTRIDGE'S ENTRY COST. Native only; the cartridge path above
     * is unchanged.
     *
     * interface_menu00_legalscreen advances g_MenuTimer by g_ClockTimer - by
     * ELAPSED VI PERIODS, not by frames - and leaves the screen when the total
     * reaches MENU_LEGALSCREEN_MENU_TIMER_MAX (241). On the cartridge the work
     * THIS function just did is not free: the PROP_LEGALPAGE load and
     * decompress, modelCalculateRwDataLen, and fileValidateSaves' EEPROM reads
     * block the game loop for about 173 VI periods before the first interface
     * call runs. Those 173 periods are spent against the same 241 budget, so
     * the legal artwork is only ever ON SCREEN for the remaining ~68.
     *
     * MEASURED, retail ROM under parallel_n64 (tools/trace/bootprobe.py,
     * baserom.u.z64 SHA1 abe01e4a...): the state spans emulator frames 74..306,
     * but currentFrameCounter is frozen at 51 and g_MenuTimer at 0 for frames
     * 74..237 with the framebuffer black - the loop is blocked, not drawing.
     * At frame 238 the stall ends and one update credits the whole backlog,
     * stepping g_MenuTimer 0 -> 173; it then advances ~1/frame to 241 at exit.
     * The artwork is above black only for frames 240..308 - 69 of the 233.
     *
     * Native loads from host memory, so that stall never happens, g_ClockTimer
     * never spikes, and all 241 periods elapse with the artwork on screen -
     * measured at 222 frames / 3.94s against the cartridge's 69 / 1.15s.
     *
     * The fix is not a sleep and not a shorter timer: the budget and the exit
     * test are the cartridge's and stay untouched. What is restored is the
     * part of the budget the cartridge had already spent by this point.
     */
    g_MenuTimer = MENU_LEGALSCREEN_ENTRY_VI_COST;
#endif
}
...
#else
    /* legal_text_ptr is uninitialized here and multiplied by zero - a
     * matching artifact the N64's readable memory absorbed */
...
#ifndef __sgi
    /* NATIVE ASSET OVERRIDE, native port only. With no override file
     * installed sl_asset_override_available() is false and the else arm is
     * the original path, unchanged.
     *
     * SCOPE: this slot is the ARTWORK LAYER ONLY - the six flat textured
     * quads this model is made of. The twelve lines of legal WORDING are not
     * in the model at all: they are font-rendered AFTER this draw from the
     * language bank, through legalpage_text_array (front.c:345) and langGet.
     * Replacing the model therefore changes zero words of text, and the black
     * behind the page is a separate G_CYC_FILL rectangle
     * (clear_framebuffer_black, above).
     *
     * lookatmtx IS this screen's transform - the lookat from (0,0,4000) built
     * above - and it is also what was just copied into mtxlist[0] for the
     * original model root, so the custom model sits under exactly the same
     * camera. NO FADE ON THIS SCREEN: interface_menu00_legalscreen advances
     * g_MenuTimer and nothing else, and the constructor issues no primitive
     * or environment colour, so the fade byte is a constant 0xFF. */
    if (sl_asset_override_available(SL_ASSET_BOOT_LEGAL_PAGE))
    {
        Mtx *slmtx = dynAllocate(sizeof(Mtx));

        matrix_4x4_f32_to_s32((f32 (*)[4]) &lookatmtx, (s32 (*)[4]) slmtx);
        gSPMatrix(renderdata.gdl++, osVirtualToPhysical(slmtx),
                  (G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW));
        renderdata.gdl = (Gfx *) sl_asset_override_emit(
            renderdata.gdl, SL_ASSET_BOOT_LEGAL_PAGE, 0xFF);
    }
    else
    {
...
#else
    /* the N64 bound relies on link-order adjacency; natively an alignment
     * gap follows the array, so bound by the array itself */
    legal_text_end = legalpage_text_array
        + sizeof(legalpage_text_array) / sizeof(legalpage_text_array[0]);
#endif
    
#ifndef __sgi
    /* NATIVE ASSET OVERRIDE, native port only - THE TEXT LAYER.
     *
     * This screen is drawn in TWO layers that do not share a coordinate
     * space. The model above is 3D geometry projected through the lookat
     * camera, in model units with +y UP. These twelve lines are 2D, in the
     * front end's 440x330 logical units with the origin at the TOP-LEFT and
     * +y DOWN, and they are submitted AFTER the model, so they land ON TOP
     * of it.
     *
     * That is exactly right for the original prop, whose five textured
     * panels carry no readable wording of their own - the words ARE this
     * layer. It is exactly wrong for a replacement page that draws its own
     * title, credit block and footer as ARTWORK: both layers would render,
     * at unrelated positions, and the page would read as doubled.
     *
     * So the layer is suppressed when, and ONLY when, the override that
     * supplies its own wording is actually resolving. The condition is the
     * SAME call the model hook above branches on, so the two cannot
     * disagree - there is no separate flag to keep in step, and the failure
     * mode of an override that fails to load is the original model AND the
     * original text, not a blank page.
     *
     * NOTHING IS DELETED OR REWORDED. legalpage_text_array is untouched,
     * every string in the language bank is untouched, and langGet still
     * resolves all twelve. With no override installed
     * sl_asset_override_available() is false and the loop below runs
     * exactly as it always has - the else arm is the original path,
     * unchanged, and SL_ASSET_OVERRIDES=0 reaches it too.
     *
     * microcode_constructor above is deliberately NOT gated: it is display
     * list state setup, not text, and the original issues it before this
     * loop whether or not the loop draws anything visible. */
    if (!sl_asset_override_available(SL_ASSET_BOOT_LEGAL_PAGE))
    {
        do
        {
...
#ifndef __sgi
    /* NATIVE ASSET OVERRIDE, native port only. With no override file
     * installed sl_asset_override_available() is false and the else arm
     * below is the original path, unchanged - same call, same order, same
     * display list. Nothing above or below this block moves: the timer, the
     * ambient light ramp, the rotation rate, the scale progression, the
     * camera and the transition to MENU_RAREWARE_LOGO are all untouched.
     *
     * ninlogoMtxf IS this screen's transform - lookat from (0,0,4000) times
     * the scaled Y rotation built above - and it is also what was just copied
     * into mtxlist[0] for the original model root. Loading it here puts the
     * custom model under exactly the same choreography, which is the point:
     * the asset is replaceable, the staging is not. `ambiantlight` rides
     * along as the fade so a custom logo fades in on the original schedule.
     *
     * The cartridge build never sees any of this - src/native is outside the
     * Makefile's globs and this arm is compiled away on IDO. */
    if (sl_asset_override_available(SL_ASSET_BOOT_NINTENDO_LOGO))
    {
        Mtx *slmtx = dynAllocate(sizeof(Mtx));

        matrix_4x4_f32_to_s32((f32 (*)[4]) &ninlogoMtxf, (s32 (*)[4]) slmtx);
        gSPMatrix(ninlogo.gdl++, osVirtualToPhysical(slmtx),
                  (G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW));
        ninlogo.gdl = (Gfx *) sl_asset_override_emit(
            ninlogo.gdl, SL_ASSET_BOOT_NINTENDO_LOGO, ambiantlight);
    }
    else
    {
...
Gfx *constructor_menu04_goldeneyelogo(Gfx *DL)
{
     ...
     * the original path, unchanged.
     *
     * THIS SCREEN IS THE REFLECTIVE CASE, and that is what makes it different
     * from the Nintendo slot. guLookAtReflect above built a reflectance basis
     * and gSPLookAt already issued it INTO THIS DISPLAY LIST, several commands
     * before the point we are standing at. The bridge command is appended
     * AFTER it, so the override draw INHERITS that resident basis exactly the
     * way subdraw's geometry would have: nothing here re-uploads, resets or
     * clobbers the lookat, and no gSPLookAt is emitted by the override path.
     * G_TEXTURE_GEN materials in the custom model therefore generate their
     * coordinates against the same basis the original letterforms used.
     *
     * logoMatrix IS this screen's transform - the lookat from (0,0,3000)
     * scaled by 1.2 - and it is also what was just copied into mtxlist[0] for
     * the original model root.
     *
     * NO FADE ON THIS SCREEN, unlike the Nintendo one. gelogolight is a
     * constant Lights1 (front.c:320): a tree-wide `grep -rn gelogolight src/`
     * finds the definition, the extern, and the two gSPLight pairs that upload
     * it, and no assignment anywhere - where ninlogolight has six per-frame
     * writes of `ambiantlight`. interface_menu04_goldeneyelogo only advances
     * g_MenuTimer, and the constructor issues no primitive or environment
     * colour. The cartridge's own menu 04 constructor agrees: it uploads the
     * two lights by movemem from fixed addresses and sets no colour at all
     * ("7F00B47C - menu 04 constructor - GoldenEye splash screen.txt"). So
     * there is nothing to ride along, and the fade byte is a constant 0xFF. */
    if (sl_asset_override_available(SL_ASSET_BOOT_GOLDENEYE_LOGO))
    {
        Mtx *slmtx = dynAllocate(sizeof(Mtx));

        matrix_4x4_f32_to_s32((f32 (*)[4]) &logoMatrix, (s32 (*)[4]) slmtx);
        gSPMatrix(gelogo.gdl++, osVirtualToPhysical(slmtx),
                  (G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW));
        gelogo.gdl = (Gfx *) sl_asset_override_emit(
            gelogo.gdl, SL_ASSET_BOOT_GOLDENEYE_LOGO, 0xFF);
    }
    else
    {
...
#if defined(SL_DEMO_BUILD) && !defined(__sgi)
    /* DEMO ACCESS POLICY - MULTIPLAYER IS VISIBLE BUT NEVER SELECTABLE.
     *
     * Rare's own disabled-entry semantic, reused rather than reinvented. This
     * one else-if carries BOTH the highlight (mission_difficulty_highlighted)
     * and the confirm, so a false condition does not enter the branch at all:
     * the entry cannot light up and cannot activate, and the cursor falls
     * through to the Single Player arm exactly as it already does on a console
     * with one controller attached. That fall-through is Rare's behaviour for
     * an unavailable Multiplayer, not something introduced here.
     *
     * Because the branch is where activation lives, this covers mouse,
     * keyboard and gamepad at once. joyGetButtonsPressedThisFrame is the only
     * activation path, and a left click reaches it as an ordinary N64 A edge
     * (src/platform/sl_input.c sets SL_BTN_A 0x8000 for the click, merged with
     * the pad's own buttons at `b |= bk`), so there is no separate mouse route
     * left to close. A greyed-but-still-clickable Multiplayer is not possible.
     *
...
Gfx* constructor_menu06_modesel(Gfx* DL)
{
     ...
     * controllers are attached - and it is applied to the "2." numeral and the
     * MULTIPLAYER label together, so the demo dims the entry the way the game
     * always has rather than inventing a disabled style. The entry stays
     * VISIBLE and legible; it simply never lights up or activates. */
    if (SL_DEMO_MULTIPLAYER_SELECTABLE && joyGetControllerCount() >= 2)
#else
...
#ifndef __sgi
/* The SAME table, keyed by the OTHER column.
 *
 * mission_folder_setup_entries carries both of the game's mission id spaces
 * side by side: `mission_num` is an SP_LEVEL (campaign order, DAM=0..EGYPT=19)
 * and `stage_id` is a LEVELID (internal map id, facility=34). They are NOT
 * interchangeable and the ranges OVERLAP, which is what makes a mixed-up
 * lookup silent instead of loud.
 *
 * pull_and_display_text_for_folder_a0 above scans `mission_num`, and both of
 * its front-end callers correctly hold an SP_LEVEL (front.c:3573 passes
 * mission_difficulty_highlighted). set_solo_and_ptr_briefing (file.c) holds a
 * LEVELID - its parameter is declared `LEVELID stage` and all three of its
 * callers pass one: boss.c:378 passes g_StageNum, and ramromreplay.c:476 and
 * :572 pass ptr_active_demofile->stagenum, declared `enum LEVELID` at
 * ramromreplay.h:14. It was calling the SP_LEVEL-keyed scan anyway (B-096).
 *
 * MEASURED before this function existed, direct boot, all twenty campaign
 * missions, one line each out of set_solo_and_ptr_briefing:
 *
 *   19 of 20   briefingpage = -1        no row has mission_num == 33, 34, ...
 *   bunker1    briefingpage = 15        LEVELID_BUNKER1 is 9, and 9 is also
 *                                       SP_LEVEL_STATUE, so the scan MATCHED
 *                                       the Statue row (stage_id=22)
 *
 * So the failure is not uniformly a -1 sentinel. Where the two ranges overlap
 * it returns a confident, wrong mission, which walks straight past the
 * `-1 < briefingpage` guard in end_of_mission_briefing (file.c:42) and would
 * record Statue's unlock and Statue's target time for a Bunker 1 completion.
 *
 * The fix belongs HERE and not in pull_and_display_text_for_folder_a0: that
 * function's own callers depend on its SP_LEVEL keying, so re-keying it would
 * break the front end to repair the boot path.
 *
 * Header rows (MISSION_HEADER) store stage_id = LEVELID_NONE, so a negative
 * argument is rejected up front rather than matching the first chapter row.
 * Loop shape, terminator test and -1 return are deliberately identical to the
 * sibling above. */
s32 sl_briefing_page_for_stage_id(s32 stage_id)
{
    int i;
    ...
    for (i = 0; mission_folder_setup_entries[i].folder_text_preset != 0 ; i++)
    {
        if (stage_id == mission_folder_setup_entries[i].stage_id)
        {
            return i;
        }
    }
...
#if defined(SL_DEMO_BUILD) && !defined(__sgi)
    /* DEMO ACCESS POLICY - THE SINGLE CHOKEPOINT, answering "every mission".
     *
     * This function is what Mission Select means by "available", and it is the
     * ONLY thing that decides it. All ten callers are in this file and every
     * one of them is presentation or gating (row snapping, row colour, the
     * initial difficulty cursor, the mission list draw, and Difficulty
     * Select) - tree-wide
     * `grep -rn get_highest_unlocked_difficulty_for_level src/` finds no other
     * caller and no writer at all. So answering here CANNOT alter a save: it
     * offers a menu row, it does not record anything as earned.
     *
     * WHY THIS SEAM AND NOT THE SAVE FILE. The cartridge's availability is
     * monotonic progress (fileIsStageUnlockedAtDifficulty over
     * selected_folder_num, below): Aztec wants every mission cleared on
     * Secret Agent, Egypt every mission on 00 Agent. Exposing the whole
     * campaign THROUGH the save would therefore mean forging nineteen
     * completions with times and medals the player never earned, and the
     * demo's save would then lie to the cheat menu, the debrief and the file
     * folder alike. Demo availability is a policy question and belongs at the
     * policy layer; the progression model below is left intact, still runs in
     * every normal build, and the demo's own save still fills in honestly as
     * the player completes missions.
     *
     * THE SET IS THE GAME'S OWN TABLE, FILTERED BY TYPE. The same row lookup
     * and the same `stage_id >= 0` test the normal arm opens with (a
     * MISSION_HEADER row stores LEVELID_NONE) decide what is a mission; there
     * is no second list of levels here, so a table change is a demo change
     * for free. Twenty rows are MISSION_PART: Dam through Egypt.
     *
     * ONE RETURN COVERS EVERY INPUT CHANNEL. The confirm tests
     * `mission_difficulty_highlighted >= 0`, and that variable can only hold an
     * index that survived the snapping loops, which consult this function -
     * mouse, keyboard and pad arrive at the same test.
     *
     * DIFFICULTY_00 keeps Agent / Secret Agent / 00 Agent all offered, which is
     * what Difficulty Select reads - for Aztec and Egypt too, which is exactly
     * "without their normal unlock requirements". Deliberately NOT
     * DIFFICULTY_007: 007 mode is unlocked CONTENT rather than a mission, and
     * the demo does not offer it - the same line the Facility-only demo drew. */
...
#ifndef __sgi
    /* Ubrief*Z arrives raw from the cartridge, and it is BIG-ENDIAN HALFWORDS
     * end to end.  "Main Menus/Solo Menus/briefings.txt" is the authority:
     * "Format of briefing files: 0x30 large - 0x0/0x2/0x4/0x6 two bytes each,
     * the Background/M/Q/Moneypenny page text IDs; 0x8 objective entries, max
     * of 10, each 0x0 two bytes text ID, 0x2 two bytes lowest difficulty for
     * objective."  That is struct BriefingData exactly, and there is not one
     * 32-bit field in it, so the whole record swaps uniformly.
     *
     * Unswapped, every field is a different string: DAM's objective text IDs
     * read 042c 052c 062c 072c instead of 2c04 2c05 2c06 2c07.  langGet takes
     * the bank from slotID >> 10, so 0x072c asks bank 1 - never loaded, hence
     * NULL - while 0x2c04 asks bank 11, which is the bank
     * load_briefing_text_for_stage has just filled.  The NULL then reaches
     * textWrap's src and is dereferenced (B-086).  The minimum difficulties
     * are wrong the same way: 0x0100/0x0200 instead of 1/2, so every
     * objective above Agent was silently skipped as well. */
    {
        extern void sl_swap_halfwords(void *p, u32 nhalves);
        sl_swap_halfwords(ptrbriefingdata, sizeof(struct BriefingData) / 2);
    }
#endif
