...
#else
        /* Same case as bg.c:3448 and bg.c:3597. image.c:2581/2609 patch this
         * word with osVirtualToPhysical(...), which is the identity natively
         * (sl_ultra_shim.c:921), so index already IS the host pointer and the
         * PHYS_TO_K0 tag would push it out of the 32-bit map. Measured on
         * Win32: genericimage->index = 0x04254180, and the tagged 0x84254180
         * faults in texFindInPool((aa)[-4]) on the first scorch-buffer draw. */
        aa = (u16 *) tconfig->index;
#endif
