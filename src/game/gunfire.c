...
#ifndef __sgi
/* B-028: the native stand-in for the three ModelRenderData templates that are
 * read as overruns off D_80035CC0 / D_80035D00 / D_80035D04+0x3c.  See the
 * note beside its definition in gun.c. */
extern const ModelRenderData sl_gun_renderdata_template;
#endif
...
#ifndef __sgi
/* ---- VIEWMODEL PROVENANCE PROBE (SL_GUN_DBG), off by default -------------
 *
 * Native-only, read-only, and inert unless the variable is set. Reports, per
 * frame, what the first-person gun path DECIDED (how many hands passed the
 * field_87F test) and what it EMITTED (the operand of every opcode-06 command
 * this function wrote). Those are the two facts the F8 record cannot supply:
 * the record names display-list operands and has no way to know which of them
 * the viewmodel produced, so "the gun is missing from the frame" could not be
 * separated from "the gun's lists were never recognised as the gun's".
 *
 * It scans only the words between this function's own entry and exit gdl, so
 * it cannot mistake a neighbour's branch for the viewmodel's.
 *
 * WHAT IT MEASURED, 2026-09-08, Dam gameplay: the viewmodel emits nineteen
 * segment-5 branches, in this order -
 *   05006138 05004038 05004958 05004a80 05004c48 05004dc0 05004fe8 05004808
 *   05004660 05004528 05004390 05004158 05005188 05005398 05005ab0 05005c78
 *   05005e00 05005b68 05005fc0
 * which is byte-for-byte the set at ord=29..47 of the owner's mark-007, in
 * the same order. That is what identifies the viewmodel inside an F8 record. */
static void sl_gun_probe(const Gfx *from, const Gfx *to, s32 hands_drawn)
{
    ...
    if (on < 0) {
        const char *e = getenv("SL_GUN_DBG");
        on = (e != NULL && e[0] != 0 && e[0] != '0');
    }
    if (!on) return;

    fprintf(stderr, "sl_gun: f%u hands=%d words=%d dl:",
            sl_frames_completed(), (int) hands_drawn,
            (int) (((const unsigned int *) to - (const unsigned int *) from) / 2));
    for (w = (const unsigned int *) from; w + 1 < (const unsigned int *) to; w += 2) {
        if ((w[0] >> 24) == 0x06) {
            fprintf(stderr, " %08x", w[1]);
            n++;
        }
    }
...
#else
    renderdata = sl_gun_renderdata_template;    /* B-028 */
#endif
...
#ifndef __sgi
    sl_gun_probe(sl_gun_from, gdl, sl_gun_hands);
#endif
    *gdlptr = gdl;
}
...
#else
    ModelRenderData renderdata = sl_gun_renderdata_template;    /* B-028 */
#endif
    ModelHeader model;
    u8 spb8[0x80];
    s32 padb4;
...
#else
    /*
     * B-029. g_DefaultCasingModelRenderData is u32[15] = 0x3c, but
     * ModelRenderData is 0x40, so this read takes cullmode (+0x3c) from past
     * the end of the array - the fourth member of the B-028 template family
     * and the last one still unguarded.
     *
     * It happens to produce the right answer on both platforms for different
     * reasons: on N64 the next symbol is dword_D_80035EEC, image 00000000; on
     * native the array is followed by four bytes of .data alignment padding
     * before ammo_related, also zero. Both give cullmode = 0 = CULLMODE_BOTH.
     * That is luck, not correctness, and one relink changes it.
     *
     * The substitution is exact, not approximate: the array's 15 words
     * {0, 1, 3, 0, ...} map field-for-field onto the template's leading 15,
     * and CULLMODE_BOTH is 0 - the same value the overrun was picking up.
     */
    ModelRenderData  render_data = sl_gun_renderdata_template;
#endif
...
{
    /*
     * B-041.  This function is declared to return a Gfx * and the decomp has
     * no return statement - it calls through and falls off the end. Its caller
     * does `gdl = set_rgba_redirect_generate_microcode(gdl, ...)`
     * (generate_ammo_total_microcode), so the display-list pointer is whatever
     * happens to be in the return register.
     *
     * On MIPS that is not luck so much as ABI: the callee's own return value
     * is already in v0 when this function's epilogue runs, so the value flows
     * through and the caller gets a correctly advanced gdl. IDO compiles it
     * that way and the ROM depends on it.
     *
     * x86 has no such guarantee - the value lands in eax only if the codegen
     * happens to leave it there - so natively the caller advanced gdl by an
     * arbitrary amount and the ammo type icon drew as a flat magenta bar
     * between the two ammo numbers.
     *
     * Guarded because ADDING the return is what would change IDO's output: the
     * matching build must keep the fall-through exactly as Rare compiled it.
     * Same class as B-029 - a decomp that leans on an implementation detail of
     * one toolchain. This is the only returnless Gfx * function in gunfire.c or
     * bondview2.c, so the class is closed here rather than merely patched.
     */
#ifdef __sgi
    microcode_generation_ammo_related(gdl, tconfig, x, y, arg4, arg5, arg6, arg7, 0xff, 0xff, 0xff, 0xff);
#else
    return microcode_generation_ammo_related(gdl, tconfig, x, y, arg4, arg5,
                                             arg6, arg7, 0xff, 0xff, 0xff, 0xff);
#endif
}
