...
#ifndef __sgi
    {
        /* B-027.  The global image bank is NOT uniformly 32-bit words, and
         * swapping it as if it were is what made the crosshair (and every
         * other bank image) draw untextured.
         *
         * Layout, per "images text and font/load global image bank.txt" and
         * "images text and font/image declarations.txt": the first 0xAC8
         * bytes are the seventeen globalDL_* display lists - word data, swap
         * them.  From s_genericimage to the end of the bank it is nothing but
         * 0xC-byte image declarations:
         *
         *     0x0  4  image ID, or rdram pointer once loaded   <- word
         *     0x4  1  width      0x8  1  bitdepth
         *     0x5  1  height     0x9  1  clamp/mirror S
         *     0x6  1  level      0xA  1  clamp/mirror T
         *     0x7  1  format     0xB  1  (pad)
         *
         * Bytes 0x4..0xB are eight independent u8 fields.  A blanket word
         * swap reverses each group of four, so texSelect read the crosshair's
         * {w=0x20,h=0x20,level=0,fmt=RGBA} as {w=0,h=0,level=0x20,fmt=0x20}:
         * zero-sized tile, bogus format, and a non-zero level that also sent
         * it down the mipmap branch.  Only the leading u32 is word data.
         *
         * The bank ends with 8 bytes of padding (0x13F8..0x1400), which the
         * `off + 0xC <= size` bound leaves alone; nothing reads it. */
        extern void sl_swap_words(void *p, u32 nwords);
        u8 *base = (u8 *) pGlobalimagetable;
        /* segment symbols carry the 0x02000000 segment VMA; the same
         * +0xFE000000 the pointer fixups below use cancels it back to a
         * bank-relative offset (0x02000ac8 -> 0xac8). */
        u32 decloff = (u32) &s_genericimage + 0xFE000000;
        u32 off;

        sl_swap_words(base, decloff / 4);

        for (off = decloff; off + sizeof(struct sImageTableEntry) <= size;
             off += sizeof(struct sImageTableEntry))
        {
            sl_swap_words(base + off, 1);
        }
    }
#endif
