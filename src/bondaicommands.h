...
#else
/* Sightline native: the DEFINED(SETUPSUBROUTINES(ID)) probe pastes '_' against
 * ')' and only resolves under IDO's non-conforming expansion order. Same
 * semantics, conforming machinery: setups that define SETUPSUBROUTINES also
 * point SL_SETUPSUBROUTINES at it (see UsetuparchZ.c); everyone else gets 0.
 * Expansion happens at the use site either way, so the timing matches the
 * probe's. */
#define SL_SETUPSUBROUTINES(ID) 0
#define isSubroutine(ID) ((ID == GAILIST_PLAY_IDLE_ANIMATION) |\
                            (ID == GAILIST_BASH_KEYBOARD) | \
                            (ID == GAILIST_ATTACK_BOND) | \
                            (ID == GAILIST_RUN_TO_BOND) | \
...
Label(lblDone))
#else
/* Sightline native: IDO fills omitted macro arguments; a conforming cpp
 * refuses the short calls. Pad to the full 49 and let the trailing ...
 * swallow the surplus - the same recipe as BITFLAG. */
#define SL_SWITCH_IMPL(VAR, \
               CASE0, CASE_VAL0, CASE_CONTENT0,\
               CASE1, CASE_VAL1, CASE_CONTENT1,\
               CASE2, CASE_VAL2, CASE_CONTENT2,\
               CASE3, CASE_VAL3, CASE_CONTENT3,\
...
IF_VA(NOT(IS_EMPTY(CASE_CONTENT0)))(IF_VA(NOT(IS_EMPTY(CASE_CONTENT1)))(Label(lblNext))                           EXPAND_ARGS_STACK(CASE_CONTENT0)(lblDone) \
Label(lblDone))
#define SWITCH(...) SL_SWITCH_IMPL(__VA_ARGS__,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,)
#endif
