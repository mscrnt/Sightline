...
unsigned long sl_mus_play_af, sl_mus_play_n;
#endif
#ifndef __sgi
/* The audio DRAM window the native ACMD interpreter is bounded to. Everything
 * an audio task touches on this build - the command list, the output buffer,
 * the DMA sample buffers - is allocated from this one heap, so it is both the
 * right window and a hard bound: an address outside it comes back as
 * SL_ACMD_ERR_DRAM rather than a wild access. */
void sl_music_heap_window(void **base, unsigned *len)
{
    *base = (void *) g_musicHeap.base;
    *len  = (unsigned) g_musicHeap.len;
}
#endif
...
#ifndef __sgi
    /* Place the audio heap at the cartridge's own physical address, so the
     * buffer addresses that end up in the ACMD command words are the ones the
     * cartridge emitted. Derived in sl_audio.c; it also removes the
     * K0_TO_PHYS mangling of the voice-path operands, because a pointer into a
     * heap below 0x20000000 survives that mask unchanged.
     *
     * The mempAllocBytesInBank call above is deliberately left in place and its
     * result deliberately unused here: every other allocation then keeps the
     * address it had, so this touches no layout but the audio heap's. Falls
     * back to the pool pointer, loudly, if the mapping is refused. */
    { extern void *sl_audio_arena(unsigned int len);
      void *a = sl_audio_arena((unsigned int) MUSIC_ALLOCATION_BYTES);
      if (a != 0) mempAddress = (u8 *) a; }
#endif
...
#ifndef __sgi
        /* the .ctl is big-endian ROM bytes and alBnkfNew is relocation-only;
         * normalise the structure graph exactly once, here, before it runs */
        { extern void sl_swap_bankfile(ALBankFile *f); sl_swap_bankfile(sfxBank); }
#endif
...
#ifndef __sgi
        { extern void sl_swap_bankfile(ALBankFile *f); sl_swap_bankfile(instrumentBank); }
#endif
...
#else
    /* PURE READ of the raw big-endian header for sizing - the temporary copy is
     * never mutated, so the full-table pass below normalises exactly once. */
    { extern u16 sl_swap_seq_count(const void *rawhdr);
      u16 hc = sl_swap_seq_count(g_musicDataTable);
      tblSegmentSize = (sizeof(RareALSeqData) * hc) + 4;
#ifdef SL_ALHEAP_TRACE
      /* The reporter is DEFINED under this flag (sl_main.c), so the call has to
       * live under it too - guarding one side and not the other leaves a live
       * reference in the production link and it becomes a generated stub. That
       * is not cosmetic: the stub count is the cheapest detector this build
       * has, and a permanently-off-by-one count detects nothing. */
      { extern void sl_seq_proof(const void *raw, u32 entsz, u32 rawsize, u32 aligned);
        sl_seq_proof(g_musicDataTable, (u32) sizeof(RareALSeqData),
                     (u32) tblSegmentSize, (u32) ALIGN16_a(tblSegmentSize)); }
#endif
    }
#endif
...
#ifndef __sgi
    /* the single semantic load boundary: header + every entry, exactly once,
     * before musicSeqFileNew (which stays relocation-only) or any other reader */
    { extern void sl_swap_seqtable(RareALSeqBankFile *f);
      sl_swap_seqtable(g_musicDataTable); }
#endif
#ifdef SL_ALHEAP_TRACE
    { extern void sl_seq_entries(const void *tbl, int n);
      sl_seq_entries(g_musicDataTable, (int) g_musicDataTable->seqCount); }
#endif
...
#ifdef SL_ALHEAP_TRACE
    { extern void sl_alheap_report(void);
      extern void sl_audio_counts(int seq, void *a, void *b, void *c);
      sl_audio_counts((int) g_musicDataTable->seqCount, g_musicXTrack1SeqPlayer,
                      g_musicXTrack2SeqPlayer, g_musicXTrack3SeqPlayer);
      sl_alheap_report(); }
#endif
...
{
#if defined(SL_ALHEAP_TRACE) && !defined(__sgi)
    /* WHEN is the track asked to play, in the audio-frame domain both sides
     * share (both count from amCreateAudioManager). The cartridge's first NOTE
     * is at audio frame 110; if this call is already late, the delay is
     * upstream in level load, and if it is early the delay is inside the
     * player. */
    { extern u32 g_AudioFrameCount; extern unsigned long sl_mus_play_af, sl_mus_play_n;
      if (sl_mus_play_n++ == 0) sl_mus_play_af = (unsigned long) g_AudioFrameCount; }
    { extern u32 g_AudioFrameCount; extern void sl_music_note(unsigned, unsigned);
      sl_music_note((unsigned) track, (unsigned) g_AudioFrameCount); }
#endif
    u32 trackSizeBytes;
    ...
    /* After the declarations: IDO is C89 and a statement above them is an
     * error, which the matching build catches and gcc does not. */
#ifdef SIGHTLINE_AUDIO_EVENTS
    { extern u32 g_AudioFrameCount;
      SL_AEV(SL_AEV_MUSIC_REQ, track, g_AudioFrameCount, 0,0,0,0,0); }
#endif
    ...
    while (alCSPGetState(g_musicXTrack1SeqPlayer))
        ;
#else
    /* THE SEQUENCE PLAYER IS STOPPED BY THE AUDIO THREAD, AND NATIVELY THERE
     * IS NO AUDIO THREAD. osCreateThread and osStartThread are no-ops in the
     * shim (sl_ultra_shim.c:871-873), so every one of amMain's iterations runs
     * from the VI retrace on the SAME thread as this code. A bare spin here
     * therefore blocks the only thing that could ever satisfy it.
     *
     * MEASURED, not reasoned: instrumented at this line, the gunbarrel's
     * musicTrack1Play(M_INTRO) read state=1 (AL_PLAYING) on all 2,000,000
     * iterations while g_AudioFrameCount stayed frozen at 679. The same probe
     * read state=0 and exited at iteration 0 for the preceding track, so it
     * was capable of reporting either outcome. That spin is why the title
     * sequence hung at the gun barrel and why the front end was unreachable.
     *
     * The pump is the platform's established answer to a game-thread block -
     * sl_pump drives sl_vi_retrace_once for exactly this reason
     * (sl_ultra_shim.c:732) - so this waits the same way rather than
     * inventing a second mechanism.
     *
     * DETERMINISM, MEASURED RATHER THAN ARGUED: a retrace spent waiting
     * advances the recorded input stream, so the helper prints "pumped N
     * retrace(s)" whenever N > 0. The headless Facility gate
     * (tools\windows\test.ps1, 300 frames, 2 runs) prints it ZERO times; a
     * front-end run prints "pumped 3" and "pumped 4". The instrument fires,
     * and the paths recorded traces take do not make it fire. */
    { extern void sl_audio_wait_seqp_stopped(void *seqp);
      sl_audio_wait_seqp_stopped(g_musicXTrack1SeqPlayer); }
#endif
...
#ifndef __sgi
    /* The sequence header's 17 big-endian words become a parsed
     * ALCMidiHdr here; normalise once, before alCSeqNew reads them. */
    { extern void sl_swap_seqheader(void *buf);
      sl_swap_seqheader(thing.seqData); }
#endif
...
void musicTrack2Play(s32 track)
{
    ...
    while (alCSPGetState(g_musicXTrack2SeqPlayer))
        ;
#else
    /* Same single-threaded deadlock as musicTrack1Play; see the note there. */
    { extern void sl_audio_wait_seqp_stopped(void *seqp);
      sl_audio_wait_seqp_stopped(g_musicXTrack2SeqPlayer); }
#endif
...
void musicTrack3Play(s32 track)
{
    ...
    /* Same single-threaded deadlock as musicTrack1Play; see the note there. */
    { extern void sl_audio_wait_seqp_stopped(void *seqp);
      sl_audio_wait_seqp_stopped(g_musicXTrack3SeqPlayer); }
#endif
