...
#else
/* Native: there is one flat address space, so a KSEG0 virtual address and the
 * physical address behind it are the SAME host pointer - which is exactly why
 * the shim makes osVirtualToPhysical the identity (sl_ultra_shim.c).  These
 * four macros were left doing the N64 arithmetic, and every DL operand built
 * through them came out 0x80000000 below the real pointer.  Measured
 * 2026-08-25: bgRenderRoomPrimary emitted the room display list at 0x48354890
 * for a list living at 0xc8354890, and the walker's seg_resolve() read the top
 * nibble as segment 8 - a segment nothing in the tree ever sets
 * (`grep -rn gSPSegment src/ --include=*.c` sets only 0,1,2,3,4,5,14,15) - so
 * every room branch resolved to NULL and was skipped.  Room geometry had never
 * rendered natively; what looked like level geometry was props and characters.
 * OS_PHYSICAL_TO_K0 is the mirror of the same mistake (options.c hands it
 * pointers that are already host pointers).  B-019. */
#define	OS_K0_TO_PHYSICAL(x)	((u32)(unsigned long)(char *)(x))
#define	OS_K1_TO_PHYSICAL(x)	((u32)(unsigned long)(char *)(x))

#define	OS_PHYSICAL_TO_K0(x)	((void *)(unsigned long)(u32)(x))
#define	OS_PHYSICAL_TO_K1(x)	((void *)(unsigned long)(u32)(x))
#endif
